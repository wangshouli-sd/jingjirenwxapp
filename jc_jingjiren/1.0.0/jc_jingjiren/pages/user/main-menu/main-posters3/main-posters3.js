...
Page({
  ...
  setcanvas() {
    ...
    // ctx.drawImage(imgUrl, 40, 40, this.data.canvasWidth - 80,  80)
    // ctx.draw()
    // })
    // 放昵称+描述
    ctx.setFontSize(14)
    ...
    ctx.clip();
    // 向画布上绘制图像  *图片圆角处理
    ctx.drawImage(that.data.avatarUrl, wrapper_x + 10, wrapper_y + 15, wrapper_width * 0.25, wrapper_width * 0.25);
    // 返回之前保存过的路径状态和属性
    ctx.restore();
    ctx.draw()
    wx.hideToast();
