...
Page({
  ...
  handleinit: function () {
    ...
    for (var i = 1; i < levelvalue; i++) {
      ...
      wx.request({
        url: app.siteInfo.siteroot +'?i=' + app.siteInfo.uniacid+'&c=entry&a=wxapp&do=index&m=jc_jingjiren&r=fr-distribution/getson',
        method: 'get',
        data: {
          openid: app.globalData.openid,
          type: i,
  ...
  onReachBottom: function () {
    ...
    if (stopRefresh) {
      ...
      let url = '?i=' + app.siteInfo.uniacid+'&c=entry&a=wxapp&do=index&m=jc_jingjiren&r=fr-distribution/getson'
      wx.request({
