...
Page({
  ...
  onLoad: function(options) {
    ...
    // 获取最新项目以及最热项目
    let type = options.type // 获取类型 1最热   2最新 
    that.setData({
      activeIndex: type == 1 ? '1' : '0'

    })

    if (type == '1') {
      that.setData({
        sliderOffset: that.data.window.windowWidth / that.data.tabs.length * that.data.activeIndex - (that.data.window.windowWidth / that.data.tabs.length - sliderWidth) / 2
    ...
    if (cateid) {
      ...
      // let rwtype = type == 1 ? '1' : '2'
      let url = app.siteInfo.siteroot + '?i=' + app.siteInfo.uniacid + '&c=entry&a=wxapp&do=index&m=jc_jingjiren&r=fr-project&type=' + type
      console.log(url)
      wx.request({
        url: url,
        method: 'get',
